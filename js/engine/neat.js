...
class Neat {
  ...
  nextGenerationSurvive(survivalPercent) {
    this.sortGeneration();
    let survivors = Math.ceil(this.brains.length * survivalPercent);
    this.brains.length = survivors;
    while (this.brains.length < this.populationSize) {
      let first = this.pickTop(0.1);
      let second = this.pickTop(survivalPercent);
      while (second == first) {
        second = this.pickTop(survivalPercent);
      }
